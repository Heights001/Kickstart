...
from engine.models.features import full_features
...
def get_rung(rung: int, config: EngineConfig) -> RungSpec:
    if rung == 0:
        return RungSpec(
            0, "rung0", rung0_features, lambda: Rung0Model(config.models.rung0, config.seed)
        )
    if rung == 1:
        return RungSpec(
            1, "rung1", full_features, lambda: Rung1Model(config.models.rung1, config.seed)
        )
    if rung == 2:
        return RungSpec(
            2,
